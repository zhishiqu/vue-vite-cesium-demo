<template>
    ...
    <select v-model="langModel">
        <option v-for="l in Object.keys(lang)" :key="l" :value="l">
            {{ l }}
        </option>
    </select>
</template>
...
import {
    drawLinesAndAirplane,
    settleBaseRadarCarRadio,
    destoryDrone,
} from "@/cesiumUtils/planeRoam";
...
const btnClickHandler = (btn) => {
    ...
    switch (id) {
            ...
            caller(
                ...
                () => {
                    ...
                    sat = new SatRoaming(viewer3D, {
                        uri: `${
                            import.meta.env.VITE_BUILD_PATH_PREFIX
                ...
                () => {
                    ...
                    timer = setTimeout(() => {
                        ...
                        Array(10)
                            .fill("")
                            .forEach((any, i) => {
                                analysisVisible(viewer3D, [
                                    poss[0],
                                    [99.45, poss[1][1] + i * 0.01, 1509.06],
                                ]);
                            });
                    }, 3000);
                },
        ...
        case "chijinxia": {
            let vvv = {
                destination: [-629130.6889394829, 4843216.325160386, 4091332.9543212038],
                direction: [0.1286117985554581, 0.41197818535405806, -0.9020714938765706],
                up: [-0.06484461890865528, 0.9111728617234152, 0.40688965513681175],
            }
            viewer3D.camera.flyTo({
                destination: new Cesium.Cartesian3(...vvv.destination),
                orientation: {
        ...
        case "shuangta": {
            let vvv = {
                destination: [-532681.9505856406, 4824832.425790638, 4126407.710268767],
                direction: [-0.8492003435580104, -0.4709096174876237, -0.2389621490081997],
                up: [-0.509165264309426, 0.6101678009591438, 0.606997519182241],
            }
            viewer3D.camera.flyTo({
                destination: new Cesium.Cartesian3(...vvv.destination),
                orientation: {
        ...
        case "nanganqu": {
            let vvv = {
                destination: [-591895.7418597052, 4841947.425828135, 4098070.3000047337],
                direction: [-0.8608831266127548, -0.502582195781092, 0.0793182122674472],
                up: [-0.3178855686645129, 0.6530012101731886, 0.6874141289988086],
            }
            viewer3D.camera.flyTo({
                destination: new Cesium.Cartesian3(...vvv.destination),
                orientation: {
